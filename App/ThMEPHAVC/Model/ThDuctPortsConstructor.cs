...
namespace ThMEPHVAC.Model
{
    ...
    public class ThDuctPortsConstructor
    {
        ...
        public ThDuctPortsConstructor(ThDuctPortsAnalysis anay_res, ThMEPHVACParam in_param)
        {
            ...
            if (port_range.Contains("侧"))
            {
                foreach (var endline in endline_segs)
                {
                    foreach (var seg in endline.segs)
                    {
                        ...
                        {
                            var p = ThMEPHVACService.Round_point(port.position, 6);
                                                    ...
                                                    ref string duct_size_info)
        {
            ...
            var proc_line = Get_real_proc_line(cur_info, pre_info, next_info, is_first, out double end_reducing_len, ref duct_size_info);
            Get_seg_info(is_last, end_reducing_len, cur_info.ports.Count, proc_line, out double step, out Queue<Point3d> positions);
            Seperate_line_by_port(step, cur_info, next_info, proc_line, port_seg.segs, is_first, positions, ref duct_size_info);
            return port_seg;
        }
        private static void Get_seg_info(bool is_last, 
                                         double end_reducing_len, 
                                         int port_num, 
                                         Line proc_line, 
                                         out double step, 
                                         out Queue<Point3d> positions)
        {
            var srt_p = proc_line.StartPoint;
            var end_p = proc_line.EndPoint;
            var dir_vec = (end_p - srt_p).GetNormal();
            //end_p -= dir_vec * end_reducing_len;
            var len = srt_p.DistanceTo(end_p);
            step = Math.Ceiling((len / port_num) / 100) * 100;
                                           ...
                                           ref string duct_size_info)
        {
            ...
            var dir_vec = ThMEPHVACService.Get_edge_direction(proc_line);
            ...
            else
            {
                ...
                if (next_info != null && duct_size_info != next_duct_size)
                    Endline_elbow_reducing(flag_p, cur_p, cur_info, pre_width, next_duct_start_width, ref duct_size_info, next_duct_size, proc_line, port_seg);
        ...
        private Point3d Create_port_fix_step(int idx, List<Port_Info> ports)
        {
            ...
            var mid_p = ThMEPHVACService.Get_mid_point(cur_p, next_p);
                                        ...
                                        ref string duct_size_info)
        {
            ...
            if (next_info != null)
            {
                ...
                string next_start_size = end_size;
                next_duct_start_width = Get_duct_width(false, air_volume, ref next_start_size);
                air_volume = Get_endline_end_air_volume(cur_info);
                Get_duct_width(false, air_volume, ref end_size);
                if (end_size != next_start_size)
                    end_reducing_len = 1000;
            }
        ...
        {
            var srt_p = cur_info.direct_edge.Source.Position;
            var end_p = cur_info.direct_edge.Target.Position;
            var dir_vec = (end_p - srt_p).GetNormal();
            var proc_line_srt_p = srt_p + dir_vec * src_shrink;
            var proc_line_end_p = end_p - dir_vec * dst_shrink;
                                       ...
                                       string duct_size)
        {
            if (next_info != null)
            {
                ...
                double cur_width = ThMEPHVACService.Calc_duct_width(false, ui_air_speed, cur_air_volume, ref duct_size);
                double next_width = ThMEPHVACService.Calc_duct_width(false, ui_air_speed, next_air_volume, ref duct_size);
        ...
        private double Get_endline_end_air_volume(Endline_Info info)
        {
            if (info.ports.Count == 0)
                return info.direct_edge.AirVolume;
            else
                return info.ports[0].air_volume;
        }
        private double Get_duct_width(bool is_first, double air_volume, ref string duct_size)
        {
            return ThMEPHVACService.Calc_duct_width(is_first, ui_air_speed, air_volume, ref duct_size);
        }
    }
}