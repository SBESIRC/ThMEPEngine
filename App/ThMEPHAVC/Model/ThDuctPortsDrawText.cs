    ...
    public class ThDuctPortsDrawText
    {
                                      ...
                                      out List<DBText> duct_size_info)
        {
            ...
            foreach (var t in text_alignment)
            {
                ...
                var text = Create_fjf_duct_info(t.is_in, elevation, scale, t.duct_size);
        ...
        private DBText Create_fjf_duct_info(bool is_in, double elevation, string scale, string duct_size)
        {
            // 不处理main在树间的情况
            string text_info = is_in ? $"{duct_size} (h" + "+X.XX" + "m)" :
                                       $"{duct_size} (h+" + elevation.ToString("0.00") + "m)";
