...
namespace ThMEPHVAC.Model
{
    public class ThDuctPortsDrawService
    {
        ...
        public static void Get_fan_dyn_block_properity( ThBlockReferenceData fan_data,
                                                        string fan_name,
                                                        out double in_width,
                                                        out double out_width)
        {
            using (var db = AcadDatabase.Active())
            {
                var properity = fan_data.CustomProperties;
                if (fan_name.Contains("轴流风机"))
                {
                    in_width = properity.Contains(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_DIAMETER) ?
                        (double)properity.GetValue(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_DIAMETER) : 0;
                    out_width = properity.Contains(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_DIAMETER) ?
                            (double)properity.GetValue(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_DIAMETER) : 0;
                }
                else
                {
                    in_width = properity.Contains(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_INLET_HORIZONTAL) ?
                        (double)properity.GetValue(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_INLET_HORIZONTAL) : 0;
                    out_width = properity.Contains(ThHvacCommon.BLOCK_DYNAMIC_PROPERTY_OUTLET_HORIZONTAL) ?
