...
namespace ThMEPHVAC.Model
{
    public class ThServiceTee
    {
        ...
        private void Insert_bypass_text_info(Duct_InParam info, string text_layer, Vector2d rot_vec, Vector3d dis_vec, bool is_vt)
        {
            ...
            if (text_info.IsNull())
                return;
            double duct_width = ThMEPHVACService.Get_width(info.room_duct_size);
            double ro_angle = rot_vec.Angle;

            Move_to_origin(ref text_info);
            Get_text_oft_vec(info.scale, duct_width, rot_vec, out Vector2d oft_v_vec, out Vector2d oft_h_vec, ref ro_angle);
        ...
        {
            string str;
            string tee_info = info.bypass_size;
            string scale = info.scale;
            string elevation = info.room_elevation;
            if (is_vt)
                str = $"{tee_info} (h+X.XXm)";
            else
            {
                string duct_info = info.other_duct_size;
                double evel = Double.Parse(elevation);
                double tee_height = ThMEPHVACService.Get_height(tee_info);
                double duct_height = ThMEPHVACService.Get_height(duct_info);
        ...
        private void Move_to_origin(ref DBText text_info)
        {
            if (!text_info.IsNull())
            {
                var textbounding = text_info.GeometricExtents;
                var p = ThMEPHVACService.Get_mid_point(textbounding.MaxPoint, textbounding.MinPoint);
                text_info.Position = new Point3d(-p.X * 0.5, -p.Y * 0.5, 0);
            }
        }
        ...
        public static ObjectId Insert_electric_valve(Vector3d fan_cp_vec, double valvewidth, double angle)
        {
            var e = new ThValve()
            {
                Length = 200,
