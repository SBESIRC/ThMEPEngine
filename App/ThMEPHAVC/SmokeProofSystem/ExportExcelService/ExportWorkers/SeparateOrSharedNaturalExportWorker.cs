...
namespace ThMEPHVAC.SmokeProofSystem.ExportExcelService.ExportWorkers
{
    public class SeparateOrSharedNaturalExportWorker : BaseExportWorker
    {
        public override void ExportToExcel(BaseSmokeProofViewModel baseModel, string systemName, ExcelWorksheet setsheet, ExcelWorksheet targetsheet, ExcelRangeCopyOperator copyoperator)
        {
            ...
            setsheet.Cells["D3"].Value = Math.Max(fontroomNaturalModel.FinalValue, fontroomNaturalModel.OpenDorrAirSupply + fontroomNaturalModel.VentilationLeakage).ToString();
            setsheet.Cells["D4"].Value = (fontroomNaturalModel.OpenDorrAirSupply + fontroomNaturalModel.VentilationLeakage).ToString();
            setsheet.Cells["D5"].Value = fontroomNaturalModel.OpenDorrAirSupply.ToString();
            setsheet.Cells["D6"].Value = fontroomNaturalModel.VentilationLeakage.ToString();
            setsheet.Cells["D7"].Value = fontroomNaturalModel.OverAk.ToString();

            double v = 0.6 * fontroomNaturalModel.OverAl / (fontroomNaturalModel.OverAk + 1);
            setsheet.Cells["D9"].Value = Math.Round(v, 2).ToString();
            setsheet.Cells["D10"].Value = Math.Min(fontroomNaturalModel.FloorNum, 3).ToString();
            setsheet.Cells["D11"].Value = (fontroomNaturalModel.SectionLength * fontroomNaturalModel.SectionWidth / 1000000).ToString();
            setsheet.Cells["D12"].Value = (fontroomNaturalModel.FloorNum < 3 ? 0 : fontroomNaturalModel.FloorNum - 3).ToString();
            setsheet.Cells["D13"].Value = fontroomNaturalModel.OverAl.ToString();
            setsheet.Cells["D14"].Value = fontroomNaturalModel.OverAk.ToString();
            setsheet.Cells["D15"].Value = fontroomNaturalModel.FinalValue.ToString();
            setsheet.Cells["D16"].Value = fontroomNaturalModel.FloorNum.ToString();
            setsheet.Cells["D17"].Value = GetLoadRange(fontroomNaturalModel.FloorType.ToString());
            setsheet.Cells["D20"].Value = fontroomNaturalModel.SectionLength.ToString();
            setsheet.Cells["D21"].Value = fontroomNaturalModel.SectionWidth.ToString();
