...
namespace ThMEPHVAC.FloorHeatingCoil
{
    class IntersectWayCalculator
    {
        ...
        Polyline GetBufferPolygon(int index, bool turn_left)
        {
            ...
            if (turn_left && index > 0 || !turn_left && index < pipe_inputs.Count - 1)
            {
                var list = shortest_way[turn_left ? index - 1 : index + 1].poly;
                var listTemp = new List<Point3d>();
                listTemp.AddRange(list);
                var points = SmoothUtils.SmoothPoints(listTemp);
                var last_way = PassageWayUtils.BuildPolyline(points);

                intersect_region = MainRegionCalculator.GetMainRegion(last_way, intersect_region, !turn_left);
                if (points.Count >= 4)
                {
                    // fix the second last seg
                    var p0 = points[points.Count - 3];
                    var p1 = points[points.Count - 2];
                    var line = new Line(p0, p0 + (p1 - p0).GetNormal() * 10000);
                    line.EndPoint = IntersectUtils.LineIntersectionPolygon(line, region);
                    var geometry = line.ToNTSLineString().Intersection(equispaced_segments[index].Last().ToNTSLineString());
                    if (geometry is Point point)
                    {
                        if (point.ToAcGePoint3d().DistanceTo(p0) > p1.DistanceTo(p0) + buffer &&
                            point.ToAcGePoint3d().DistanceTo(p0) < 2 * p1.DistanceTo(p0) + buffer)
                        {
                            points[points.Count - 2] = point.ToAcGePoint3d();
                            points[points.Count - 2] += (p0 - p1).GetNormal() * buffer;
                            points[points.Count - 1] += (points[points.Count - 2] - p1);
                        }
                    }
                    //// fix the fist seg
                    //var dp = points[0] - pipe_inputs[index].pin;
                    //dp = dp / dp.Length * (buffer - dp.Length);
                    //points[0] += dp;
                    //points[1] += dp;
        ...
        List<Point3d> GetBufferPolyline(int index, Polyline buffer_polygon, bool turn_left)
        {
            ...
            if (inter.Count > 0)
            {
                ...
                if (!is_start_closer_to_pin && !inter_with_last_seg)
                    return null;
        ...
        Polyline AdjustBufferRoom(int index)
        {
            ...
            {
                var pre = PassageWayUtils.GetSegIndexOnPolygon(pipe_inputs[index].pout, points);
                var next = (pre + 1) % points.Count;
                if (pipe_inputs[index].pout.DistanceTo(points[pre]) <= room_buffer + pipe_inputs[index].out_buffer + 1) 
                {
                    var ppre = (pre - 1 + points.Count) % points.Count;
                    var old_pre_point = points[pre];
                    ...
                    {
                        points[pre] = pipe_inputs[index].pout + (old_pre_point - pipe_inputs[index].pout).GetNormal() * (buffer / 4 + room_buffer);
                        points[ppre] += (points[pre] - old_pre_point);
                    }
                }
                else if(pipe_inputs[index].pout.DistanceTo(points[next]) <= room_buffer + pipe_inputs[index].out_buffer + 1)
                {
                    var nnext = (next + 1) % points.Count;
                    var old_next_point = points[next];
                    ...
                    if (dir == pipe_inputs[index].end_dir)
                    {
                        points[next] = pipe_inputs[index].pout + (old_next_point - pipe_inputs[index].pout).GetNormal() * (buffer / 4 + room_buffer);
                        points[nnext] += (points[next] - old_next_point);
                    }
                }
            }
        ...
        void ToughFixIntersectionMain(int index,bool turn_left)
        {
            if (turn_left && index > 0 || !turn_left && index < pipe_inputs.Count - 1)
            {
                ...
                //PassageShowUtils.ShowPoints(shortest_way[last_index].poly);
                if (pipe_inputs[last_index].is_out_free == false) return;
                var main_pipe = shortest_way[index].Buffer(4);
                main_pipe = PassageWayUtils.Buffer(main_pipe, -10).First();
                //PassageShowUtils.ShowEntity(main_pipe);
                int i = 0;
                ...
                if (i > 1)
                {
                    ...
                    var next = (pre + 1) % points.Count;
                    var line = new Line(points[pre], points[next]);
                    shortest_way[last_index].poly[i - 1] = line.GetClosestPointTo(shortest_way[last_index].poly[i - 1], false);
