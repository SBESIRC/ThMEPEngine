...
namespace ThMEPHVAC.FloorHeatingCoil.Heating
{
    class DataPreprocess
    {
        ...
        public int FindMainRegion()
        {
            ...
            for (; mainRegionId < this.RegionObbs.Count; mainRegionId++)
            {
                if (RegionObbs[mainRegionId].Contains(rawData1.WaterSeparator.StartPts[0]))
                {
                    break;
                }
            }
        ...
        public void CreateGraph()
        {
            ...
            SingleDoor zeroDoor = new SingleDoor(doorCount, RegionList[MainRegionId], RegionList[MainRegionId], zeroDoorObb);
        ...
        public void ResizeTheDoor() 
        {
            ...
            {
                int doorLineIndex = DoorList[i].DownLineIndex;
                Polyline clearedPl = DoorList[i].DownstreamRegion.ClearedPl;
                ...
                if (pt0.DistanceTo(DoorList[i].DownFirst) < Parameter.SuggestDistanceWall * 1.5) 
                {
                    ...
                    DoorList[i].UpSecond = DoorList[i].UpSecond + ex;
                }
                if (pt1.DistanceTo(DoorList[i].DownSecond) < Parameter.SuggestDistanceWall * 1.5)
                {
                    ...
                    DoorList[i].UpFirst = DoorList[i].UpFirst + ex;
                }
        ...
        public Line DoorToPoint3d(Polyline doorObb, Polyline regionObb)
        {
            ...
            var trimedLine = doorObb.Trim(regionObb).OfType<Polyline>().ToList().FindByMax(x => x.Length);
        ...
        public Line WaterSeparatorToDoorLine(Polyline regionObb) 
        {
            Line doorLine = new Line(new Point3d(0, 0, 0), new Point3d(0, 0, 0));
        ...
        {
            Line downLine = WaterSeparatorToDoorLine(sd.DownstreamRegion.ClearedPl);
            DrawUtils.ShowGeometry(downLine, "l1DoorLine", 3, lineWeightNum: 30);
    ...
    class Connection 
    {
        ...
        public int DoorId;
        public int RegionId;
        public Connection(Polyline doorObb,int regionId) 
        {
