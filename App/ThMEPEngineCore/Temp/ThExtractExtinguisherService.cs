...
{
    public class ThExtractExtinguisherService : ThExtractService
    {
        public List<DBPoint> Extinguishers { get; set; }
        public ThExtractExtinguisherService()
        {
        ...
        public override void Extract(Database db,Point3dCollection pts)
        {
            using (AcadDatabase acadDatabase = AcadDatabase.Use(db))
            {
                ...
                foreach (var ent in acadDatabase.ModelSpace)
                {
                    ...
                    {
                        if (IsExtinguisherBlkName(br.GetEffectiveName()))
                        {
                            blocks.Add(br);
                        }
                    }
                ...
                foreach (var br in blocks)
                {
                    ...
                    Extinguishers.Add(dbPoint);
                }
            }
        }

        private bool IsExtinguisherBlkName(string blkName)
        {
            string queryChars = "-新";
            int index = blkName.LastIndexOf(queryChars);
            if(index + queryChars.Length == blkName.Length)
            {
                return blkName.Contains("灭火器");
            }
            return false;
        }
