    ...
    public class ConnectMethodService
    {
        ...
        public Polyline CennectToPoint(Polyline wiring, BlockReference block, double range, List<Point3d> connectPts)
        {
            ...
            if (pts.Count > 0)
            {
                ...
                return UpdateWiring(wiring, connectPt, cutPoint);
            }
        ...
        /// <returns></returns>
        private Polyline UpdateWiring(Polyline wiring, Point3d connectPt, Point3d turnPt)
        {
            if (wiring.StartPoint.DistanceTo(connectPt) < wiring.EndPoint.DistanceTo(connectPt))
            {
                wiring.ReverseCurve();
            }
            ...
            for (int i = 0; i < wiring.NumberOfVertices - 1; i++)
            {
                ...
                if (line.GetClosestPointTo(turnPt, false).DistanceTo(pt) < tol)
                {
