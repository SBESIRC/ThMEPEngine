...
namespace ThMEPEngineCore.AlarmSensorLayout.Test
{
    class TestCmd
    {
        ...
        public void THASLM()
        {
            using (AcadDatabase acadDatabase = AcadDatabase.Active())
            {
                ...
                var detectList = getPoly(frame, "AI-探测区域", transformer, false);
                ...
                spaceDivider.Compute(frame, layouts);
                //输入参数
                InputArea input_Area = new InputArea(frame, layouts, holeList, wallList, columnList, null, detectList, spaceDivider.UCSs);
                var equipmentParameter = new EquipmentParameter(radius, BlindType.CoverArea);
                ...
                var color = 0;
                foreach (var UCS in spaceDivider.UCSs.Keys)
                {
                    acadDatabase.ModelSpace.Add(UCS);
                }
                //输出参数
                //输出参数
                var blinds = sensorOpt.Blinds;
                ...
                ShowPoints(layoutPoints);
                ShowBlind(blinds);
                //for (int i = 0; i < sensorOpt.Positions.Count; i++)
                //{
                //    color = (color + 1) % 8;
                //    var dbpos = sensorOpt.PlacePoints[i];
                //    var circle = new Circle(dbpos, Vector3d.ZAxis, 100);
                //    circle.ColorIndex = color;
                //    var id = acadDatabase.ModelSpace.Add(circle);
                //    //if (sensorOpt.Resp[i] is Polygon)
                //    //{
                //    //    var dbpolygon = (sensorOpt.Resp[i] as Polygon).ToDbMPolygon();
                //    //    dbpolygon.ColorIndex = color;
                //    //    id = acadDatabase.ModelSpace.Add(dbpolygon);
                //    //}
                //    //else if(sensorOpt.Resp[i] is MultiPolygon)
                //    //{
                //    //    foreach(Polygon poly in sensorOpt.Resp[i] as MultiPolygon)
                //    //    {
                //    //        var dbpolygon = poly.ToDbMPolygon();
                //    //        dbpolygon.ColorIndex = color;
                //    //        id = acadDatabase.ModelSpace.Add(dbpolygon);
                //    //    }
                //    //}
                //}
                //foreach(var detect in sensorOpt.Detect)
                //{
                //    var dbdetect = detect.ToDbMPolygon();
                //    dbdetect.ColorIndex = 3;
                //    acadDatabase.ModelSpace.Add(dbdetect);
                //}

                foreach (var line in sensorOpt.lines)
                {
                    var dbline = line.ToDbLine();
                    acadDatabase.ModelSpace.Add(dbline);
                }
            }
        }
