...
namespace ThMEPEngineCore.Algorithm.AStarAlgorithm
{
    ...
    {
        public List<Point> AdjustPath<T>(List<Point> path, Map<T> map)
        {
        ...
        /// <returns></returns>
        private List<Point> ReduceInflectionPoint<T>(List<Point> inflectionPoints, Map<T> map)
        {
        ...
        /// <param name="routePlanData"></param>
        /// <returns></returns>
        private bool AdjustInflectionPoint<T>(List<Point> inflectionPoints, Point firPt, Map<T> map, List<Point> midPts, out Point useNextPt)
        {
        ...
        /// <param name="routePlanData"></param>
        /// <returns></returns>
        private bool CheckAdjustPoint<T>(Point firPt, List<Point> midPts, Point nextPt, Map<T> map)
        {
            int yValue = midPts.Where(x => x.Y == firPt.Y).Count() == 0 ? firPt.Y : nextPt.Y;
