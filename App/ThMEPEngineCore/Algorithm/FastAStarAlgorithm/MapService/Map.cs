...
namespace ThMEPEngineCore.Algorithm.FastAStarAlgorithm.MapService
{
    public class Map<T>
    {
        double step = 800;
        ...
        public ThCADCoreNTSSpatialIndex SwapSpatialIndex;
        ...
        /// </summary>
        /// <param name="holes"></param>
        public void SetSwap(List<Polyline> _swaps)
        {
            swaps = _swaps.SelectMany(x => x.Buffer(avoidHoleDistance).Cast<Polyline>()).ToList();

            DBObjectCollection dbObjColl = new DBObjectCollection();
            foreach (var h in swaps)
            {
                var MPolygon = h.ToNTSPolygon().ToDbMPolygon();
                dbObjColl.Add(MPolygon);
            }

            SwapSpatialIndex = new ThCADCoreNTSSpatialIndex(dbObjColl);
        }
        ...
        public bool IsSwap(Point cell)
        {
            Point3d cellPt = mapHelper.TransformMapPoint(cell);
            var polyline = CreatePolyline(cellPt);
            var isSwap = SwapSpatialIndex.Intersects(polyline, true);
            polyline.Dispose();
            return isSwap;
        }
