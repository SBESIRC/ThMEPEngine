...
{
  public class ThExternalTagsDbExtension : ThDbExtension, IDisposable
    {
        public List<Entity> ExternalTags { get; set; }
        public ThExternalTagsDbExtension(Database db) : base(db)
        {
            LayerFilter = ThExternalTagsLayerManager.XrefLayers(db);
            ExternalTags = new List<Entity>();
        }
        public void Dispose()
        {
        }
            ...
            using (AcadDatabase acadDatabase = AcadDatabase.Use(HostDb))
            {
                foreach (var ent in acadDatabase.ModelSpace)
                {
                    if (ent is BlockReference blkRef)
                    {
                        ...
                        ExternalTags.AddRange(BuildElementCurves(blkRef, mcs2wcs));
                    }
                }
            }
        }
