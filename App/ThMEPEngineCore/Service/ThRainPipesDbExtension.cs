...
{
  public  class ThRainPipesDbExtension : ThDbExtension, IDisposable
    {
       
        public List<Curve> Polylines { get; set; }
        public ThRainPipesDbExtension(Database db) : base(db)
        {
            LayerFilter = ThRainPipesLayerManager.XrefLayers(db);
                    ...
                    if (IsBuildElementBlock(blockTableRecord))
                    {
                        foreach (var objId in blockTableRecord)
                        {
                            ...
                            else if ((dbObj is Polyline circle1))
                            {
                                ...
                                {
                                    circles.Add(circle1.GetTransformedCopy(matrix) as Line);
                                }
                            }
                        }
