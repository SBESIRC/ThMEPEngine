...
{
    public class ThDoorStoneDbExtension : ThStructureDbExtension, IDisposable
    {
        public List<Curve> Stones { get; set; }
        public ThDoorStoneDbExtension(Database db) : base(db)
        {
            Stones = new List<Curve>();
        }
        public void Dispose()
        {
        }
            ...
            using (AcadDatabase acadDatabase = AcadDatabase.Use(HostDb))
            {
                foreach (var ent in acadDatabase.ModelSpace)
                {
                    if (ent is BlockReference blkRef)
                    {
                        ...
                        Stones.AddRange(BuildElementCurves(blkRef, mcs2wcs));
                    }
                }
            }
        }
