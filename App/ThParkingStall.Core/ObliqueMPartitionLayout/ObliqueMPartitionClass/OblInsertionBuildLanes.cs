        ...
        private double GenerateLaneBetweenTwoBuilds(ref GenerateLaneParas paras)
        {
            ...
            for (int i = 0; i < BuildingBoxes.Count - 1; i++)
            {
                for (int j = i + 1; j < BuildingBoxes.Count; j++)
                {
                    ...
                    if (!IsConnectedToLane(gline,IniLanes)) continue;
                    double dis_connected_double = 0;
                    if (IsConnectedToLaneDouble(gline,IniLanes)) dis_connected_double = DisCarAndHalfLane;
                    bool quit = false;
                    ...
                    //生成参数赋值
                    if (IsConnectedToLane(gline,IniLanes) || IsConnectedToLane(gline, false,IniLanes))
                    {
            ...
            if (paras.LanesToAdd.Count > 0)
            {
                switch (LayoutMode)
                {
                    ...
                    case ((int)LayoutDirection.FOLLOWPREVIOUS):
                        {
                            if (ParentDir != Vector2D.Zero && paras.LanesToAdd.Count > 0 && (IsPerpVector(ParentDir, Vector(paras.LanesToAdd[0].Line)) || IsParallelVector(ParentDir, Vector(paras.LanesToAdd[0].Line))))
                            {
                                generate_lane_length *= LayoutScareFactor_ParentDir;
                            }
                            break;
                        }
                    case ((int)LayoutDirection.HORIZONTAL):
                        {
                            ...
                            if (generate_lane_length > 0 && paras.LanesToAdd.Count > 0 && !IsHorizontalLine(paras.LanesToAdd[0].Line, 45))
                            {
                                generate_lane_length *= LayoutScareFactor_Adjacent;
                            }
                            break;
                        }
                    case ((int)LayoutDirection.VERTICAL):
                        {
                            ...
                            if (generate_lane_length > 0 && paras.LanesToAdd.Count > 0 && !IsVerticalLine(paras.LanesToAdd[0].Line, 45))
                            {
                                generate_lane_length *= LayoutScareFactor_Adjacent;
                            }
                            break;
