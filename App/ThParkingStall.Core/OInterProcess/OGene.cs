...
namespace ThParkingStall.Core.OInterProcess
{
    ...
    public class OGene
    {
        public int Type { get; set; }//基因类型，为分割线基因（0），或是边界基因（1），可扩展
        ...
        public OGene(int type,double value,bool isLowerBound = false)//仅包含一个基因的组
        {
            Type = type;
            DDNA dDNA;
            switch (Type)
            {
                case 0:
                    dDNA = new DDNA(0,value, isLowerBound);
                    break;
                default:
    ...
    public class DDNA//double DNA
    {
        ...
        public DDNA(int type,double value,bool isLowerBound)
        {
            Type = type;
            Value = value;
