...
namespace ThMEPStructure.StructPlane.Print
{
    internal class ThSlabPatternTableBuilder
    {
        ...
        {
            var results = CreateTable();
            MoveTo(results);
            return results;
        }

        private void MoveTo(DBObjectCollection objs)
        {
        ...
        private DBObjectCollection CreateTable()
        {
            //在原点处创建
            using (var acadDb = AcadDatabase.Use(TblParameter.Database))
            {
        ...
        private DBText CreateDBtext(string content)
        {
            ...
            dbText.TextStyleId = DbHelper.GetTextStyleId(ThPrintStyleManager.THSTYLE3);
