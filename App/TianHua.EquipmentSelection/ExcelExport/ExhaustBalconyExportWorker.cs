...
{
    class ExhaustBalconyExportWorker : BaseExportWorker
    {
        public override void ExportToExcel(IFanModel exhaustfanmodel, Worksheet setsheet, Worksheet targetsheet, FanDataModel fandatamodel, ExcelRangeCopyOperator copyoperator)
        {
            ...
            setsheet.SetCellValue("D8", FuncStr.NullToStr(Math.Round(ExhaustModelCalculator.GetOverfloorCalcAirVolum(exhaustModel))));
            setsheet.SetCellValue("D9", FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FireOpening.NullToDouble()+ exhaustModel.Spill_OpenBalcony.NullToDouble())));
            ...
            setsheet.SetCellValue("D20", FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FuelBalcony.NullToDouble())));
            setsheet.SetCellValue("D21", FuncStr.NullToStr(Math.Round(exhaustModel.Spill_BalconySmokeBottom.NullToDouble())));
            setsheet.SetCellValue("D22", FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FireOpening.NullToDouble())));
            setsheet.SetCellValue("D23", FuncStr.NullToStr(Math.Round(exhaustModel.Spill_OpenBalcony.NullToDouble())));
            ...
            switch (exhaustModel.ExhaustCalcType)
            {
                ...
                case "走道回廊-房间内和走道或回廊都设置排烟":
                    ...
                    copyoperator.CopyRangeToOtherSheet(setsheet, "A1:D28", targetsheet);
                    break;
                case "空间-净高大于6m":
                case "空间-汽车库":
        ...
        {
            setsheet.SetCellValue("D24", FuncStr.NullToStr(Math.Round(exhaustModel.CoveredArea.NullToDouble())));
            setsheet.SetCellValue("D25", FuncStr.NullToStr(Math.Round(exhaustModel.HeatReleaseRate.NullToDouble())));
            setsheet.SetCellValue("D26", FuncStr.NullToStr(Math.Round(exhaustModel.SmokeThickness.NullToDouble())));
            setsheet.SetCellValue("C27", exhaustModel.SmokeFactorOption);
            setsheet.SetCellValue("D27", FuncStr.NullToStr(Math.Round(exhaustModel.SmokeFactorValue.NullToDouble(), 1)));
            setsheet.SetCellValue("D28", FuncStr.NullToStr(Math.Round(exhaustModel.MaxSmokeExtraction.NullToDouble())));
        }
