...
namespace TianHua.FanSelection.ExcelExport
{
    class ExhaustBalconyExportWorker : BaseExportWorker
    {
        public override void ExportToExcel(IFanModel exhaustfanmodel, ExcelWorksheet setsheet, ExcelWorksheet targetsheet, FanDataModel fandatamodel, ExcelRangeCopyOperator copyoperator)
        {
            ...
            setsheet.Cells["D8"].Value = FuncStr.NullToStr(Math.Round(ExhaustModelCalculator.GetOverfloorCalcAirVolum(exhaustModel)));
            setsheet.Cells["D9"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FireOpening.NullToDouble() + exhaustModel.Spill_OpenBalcony.NullToDouble()));
            ...
            setsheet.Cells["D20"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FuelBalcony.NullToDouble()));
            setsheet.Cells["D21"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.Spill_BalconySmokeBottom.NullToDouble()));
            setsheet.Cells["D22"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.Spill_FireOpening.NullToDouble()));
            setsheet.Cells["D23"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.Spill_OpenBalcony.NullToDouble()));
            ...
            switch (exhaustModel.ExhaustCalcType)
            {
                ...
                case "走道回廊-房间内和走道或回廊都设置排烟":
                    ...
                    copyoperator.CopyRangeToOtherSheet(setsheet, 1, 1, 28, 4, targetsheet);
                    break;
                case "空间-净高大于6m":
                case "空间-汽车库":
        ...
        {
            setsheet.Cells["D24"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.CoveredArea.NullToDouble()));
            setsheet.Cells["D25"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.HeatReleaseRate.NullToDouble()));
            setsheet.Cells["D26"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.SmokeThickness.NullToDouble()));
            setsheet.Cells["C27"].Value = exhaustModel.SmokeFactorOption;
            setsheet.Cells["D27"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.SmokeFactorValue.NullToDouble(), 1));
            setsheet.Cells["D28"].Value = FuncStr.NullToStr(Math.Round(exhaustModel.MaxSmokeExtraction.NullToDouble()));
        }
