...
namespace ThCADCore.NTS
{
    public static class ThCADCoreNTSBlockReferenceExtension
    {
        ...
        /// <param name="ecs2Wcs"></param>
        /// <returns></returns>
        public static Polyline ToOBBNoTCH(this BlockReference br)
        {
            var entities = new DBObjectCollection();
            ThBlockReferenceExtensions.Burst(br, entities);
            var filters = entities.OfType<Curve>().Where(e => e.Visible && e.Bounds.HasValue && !e.GetRXClass().DxfName.ToUpper().StartsWith("TCH")).ToCollection();
            if (filters.Count > 0)
            {
                return filters.GetMinimumRectangle();
            }
