    ...
    class ThFEILineExtension
    {
        ...
        private static List<Tuple<int, int, string>> LineRelationship(List<Line> lines, double DistGap2Extend, double DistGap2Merge, double AngleTolerance)
        {
            ...
            lines.ForEach(o =>
            {
                ...
                {
                    if(!spInLine)
                        spInLine = sp.PointInLineSegment(line, 0.000001, 0.00001);
                    if(!epInLine)
                        epInLine = ep.PointInLineSegment(line, 0.000001, 0.00001);
                }
                cross.ForEach(z =>
                {
                    var linePair = new List<int>();
                    ...
                    {
                        var intersect = o.Intersect(z, Intersect.ExtendThis).FirstOrDefault();
                        string type = "";
                        if (intersect == null || intersect.PointInLineSegment(o, 0.00001, 0.0001))
                        {
                            type = "";
                        }
                        else 
                        {
                            bool isSpExtend = intersect.DistanceTo(sp) < intersect.DistanceTo(ep);
                            if ((isSpExtend && spInLine) || (!isSpExtend && epInLine))
                                type = "";
                            else
                                type = "cross-extend";
                        }
                        if(!string.IsNullOrEmpty(type))
                            lists.Add(Tuple.Create(linePair.Min(), linePair.Max(), type));
                ...
                if (lists.Find(o => lists.IndexOf(o) < i && o == tuple) == null)
                {
                    switch (tuple.Item3)
                    {
                        case "cross":
                            ...
                            {
                                var line1 = lines[tuple.Item1];
                                if (!PointInOtherLine(lines, line1, line1.StartPoint) && !PointInOtherLine(lines, line1, line1.EndPoint))
                                    lines[tuple.Item1] = BreakAtIntersect(lines[tuple.Item1], intersectPts.First(), DistGap2Extend);
                                var line2 = lines[tuple.Item2];
                                if (!PointInOtherLine(lines, line2, line2.StartPoint) && !PointInOtherLine(lines, line2, line2.EndPoint))
        ...
        private static bool PointInOtherLine(List<Line> otherLines, Line line, Point3d point) 
        {
            bool inLine = false;
            if (otherLines == null || otherLines.Count < 1)
                return inLine;
            foreach (var checkLine in otherLines) 
            {
                if (inLine)
                    break;
                if (line.StartPoint.DistanceTo(checkLine.StartPoint) < 1 && line.EndPoint.DistanceTo(checkLine.EndPoint) < 1)
                    continue;
                if (line.StartPoint.DistanceTo(checkLine.EndPoint) < 1 && line.EndPoint.DistanceTo(checkLine.StartPoint) < 1)
                    continue;
                inLine = point.PointInLineSegment(checkLine, 0.0001, 0.00001);
            }
            return inLine;
        }
