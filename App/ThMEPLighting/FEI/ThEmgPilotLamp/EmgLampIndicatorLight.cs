...
namespace ThMEPLighting.FEI.ThEmgPilotLamp
{
    class EmgLampIndicatorLight
    {
        ...
        public EmgLampIndicatorLight(List<Polyline> columns, List<Polyline> walls, IndicatorLight indicator)
        {
            ...
            foreach (var line in exitLines)
            {
                ...
                foreach (var item in lineNodes)
                {
                    ...
                    foreach (var node in item.nodeDirections)
                    {
                        ...
                        double test = GraphUtils.GetRouteDisToEnd(route);
                        var newRoute = new NodeDirection(node.nodePointInLine, null, test, node.outDirection, node.graphNode);
        ...
        private void CalaMainLayout()
        {
            ...
            foreach (var lineInfo in lineGrapheNodes)
            {
                ...
                //GetLightLayoutPlanB(lineInfo, inWalls, inColumns);
                //节点处判断是否需要添加吊装指示
        ...
        private bool CheckAddHosting(NodeDirection nodeDirection,Vector3d sideLineDir,out bool isTwoSide) 
        {
            ...
            foreach (var dir in nodeDirection.inDirection)
            {
                ...
                foreach (var item in addDirs)
                {
                    if (!addToDir)
                        break;
                    var dirAngle = item.GetAngleTo(dir);
