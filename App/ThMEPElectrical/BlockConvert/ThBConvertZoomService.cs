        ...
        public void Zoom(ThBConvertCompareModel model, double scale)
        {
            ...
            using (var activeDb = AcadDatabase.Active())
            {
                if (model.Database.Equals(activeDb.Database))
                {
                    ...
                    {
                        Highlight(activeDb, model.SourceId, scale);
                    }
                    ...
                    {
                        Highlight(activeDb, model.TargetIdList[0], scale);
                    }
                }
            }
        }

        private void Highlight(AcadDatabase activeDb, ObjectId objectId, double scale)
        {
