namespace ThMEPWSS.ReleaseNs.RainSystemNs
{
    ...
    public class ThRainSystemServiceGeoCollector3
    {
        ...
        static bool isRainLayer(string layer) => layer is QUOTATIONABSORBENT or THESAURUSABSENCE or ELECTROMAGNETIC;
        private void handleEntity(Entity entity, Matrix3d matrix, List<KeyValuePair<Geometry, Action>> fs)
        {
            ...
            }
            {
                if (isRainLayer(entity.Layer))
                {
                    ...
                    else if (dxfName is CONVENTIONALIZED && entity.Layer is THESAURUSABSENCE)
                    {
                        var lines = entity.ExplodeToDBObjectCollection().OfType<Line>().Where(x => x.Length > INATTENTIVENESS).ToList();
                        if (lines.Count > QUOTATIONSHAKES)
                        {
            ...
            {
                if (entity.Layer is THESAURUSABSENCE)
                {
                    foreach (var c in entity.ExplodeToDBObjectCollection().OfType<Circle>().Where(IsLayerVisible))
                    {
                        ...
                        else if (THESAURUSACCEDE <= c.Radius && c.Radius < distinguishDiameter)
                        {
                            var r = c.Bounds.ToGRect().TransformBy(matrix);
                            reg(fs, r, condensePipes);
                            return;
            ...
            else if (dxfName == THESAURUSALTHOUGH && entity.Layer is ELECTROMAGNETIC)
            {
            ...
            {
                if (entity.Layer is THESAURUSADVANCE)
                {
                    dynamic o = entity.AcadObject;
                ...
                else
                {
                    ...
                    var colle = entity.ExplodeToDBObjectCollection();
                    {
                        ...
                        foreach (var seg in colle.OfType<Line>().Where(x => x.Length > QUOTATIONSHAKES).Where(x => isRainLayer(x.Layer)).Where(IsLayerVisible).Select(x => x.ToGLineSegment().TransformBy(matrix)))
                        {
                            reg(fs, seg, labelLines);
                        }
                    }
        ...
        private void handleBlockReference(BlockReference br, Matrix3d matrix, List<KeyValuePair<Geometry, Action>> fs)
        {
            ...
            if (IsLayerVisible(br))
            {
                ...
                if (name.Contains(THESAURUSANAESTHETIC))
                {
                ...
                if (name.ToUpper() is ANAESTHESIOLOGY || name.ToUpper().EndsWith(PLEASURABLENESS))
                {
                    var bd = br.Bounds.ToGRect().TransformBy(matrix);
                    reg(fs, bd, gravityWaterBuckets);
                    return;
                }
                if (name.ToUpper() is QUOTATIONGENERAL or ANAESTHETICALLY || name.ToUpper().EndsWith(THESAURUSAMIABLE) || name.ToUpper().EndsWith(ANAKHRONIZESTHAI))
                {
                    var bd = br.Bounds.ToGRect().TransformBy(matrix);
    ...
    public partial class RainDiagram
    {
        ...
        public static bool CollectRainData(Point3dCollection range, AcadDatabase adb, out List<StoreyInfo> storeysItems, out List<RainDrawingData> drDatas, bool noWL = THESAURUSABDOMEN)
        {
            ...
            }
            {
                ...
                foreach (var storey in thStoreys)
                {
                    ...
                    switch (storey.StoreyType)
                    {
                        ...
                        case ThMEPEngineCore.Model.Common.StoreyType.StandardStorey:
                        case ThMEPEngineCore.Model.Common.StoreyType.NonStandardStorey:
                            storey.Numbers.ForEach(i => wsdStoreys.Add(new ThwSDStoreyItem() { Storey = i + UNINTENTIONALLY, Boundary = bd, }));
                            break;
                        case ThMEPEngineCore.Model.Common.StoreyType.Unknown:
                        default:
                            break;
...
#pragma warning disable
        ...
        public static List<RainGroupedPipeItem> GetRainGroupedPipeItems(List<RainDrawingData> drDatas, List<StoreyInfo> thStoreys, out List<int> allNumStoreys, out List<string> allRfStoreys, out OtherInfo otherInfo)
        {
            ...
            var minS = storeys.Where(IsNumStorey).Select(GetStoreyScore).Where(x => x > QUOTATIONSHAKES).Min();
            var maxS = storeys.Where(IsNumStorey).Select(GetStoreyScore).Where(x => x > QUOTATIONSHAKES).Max();
            ...
            }
            {
                for (int i = QUOTATIONSHAKES; i < storeysItems.Count; i++)
                {
                    if (storeysItems[i].Labels.Count == THESAURUSACCESSION)
                    {
                        var storey = storeysItems[i].Labels[QUOTATIONSHAKES];
                        var _s = getHigherStorey(storey);
                        if (_s != null)
                        {
                            var drData = drDatas[i];
                            for (int i1 = QUOTATIONSHAKES; i1 < drData.RoofWaterBuckets.Count; i1++)
                            {
                                var kv = drData.RoofWaterBuckets[i1];
                                if (kv.Value == THESAURUSAMBITIOUS)
                                {
            ...
            string getWaterWellLabel(string label)
            {
                ...
                var ret = _getWaterWellLabel(label, ACCLIMATIZATION);
                ret ??= _getWaterWellLabel(label, THESAURUSANAEMIC);
                return ret;
            }
            int getWaterWellId(string label)
            {
                ...
                var ret = _getWaterWellId(label, ACCLIMATIZATION);
            ...
            int getRainPortId(string label)
            {
                ...
                var ret = _getRainPortId(label, ACCLIMATIZATION);
                if (ret == -THESAURUSACCESSION)
                {
                    ret = _getRainPortId(label, THESAURUSANAEMIC);
                }
                return ret;
            }
            ...
            {
                if (getHasRainPort(label) || getHasWaterSealingWell(label)) return THESAURUSABDOMEN;
            ...
            {
                if (IsY1L(label)) return QUOTATIONSHAKES;
                int _getFDCount()
                {
                    for (int i = QUOTATIONSHAKES; i < storeysItems.Count; i++)
                    {
            ...
            var iRF2 = allStoreys.IndexOf(THESAURUSACCESSORY);
            {
                ...
                foreach (var kv in pipeInfoDict)
                {
                    ...
                    if (IsNL((item.Label)))
                    {
                        if (!item.Items.TryGet(iRF).Exist)
                            item.HasLineAtBuildingFinishedSurfice = THESAURUSABDOMEN;
                    }
                    if (iRF >= THESAURUSACCIDENT)
                    {
                        if (!item.Items[iRF].Exist && item.Items[iRF - THESAURUSACCESSION].Exist && item.Items[iRF - THESAURUSACCIDENT].Exist)
                        {
                            var hanging = item.Hangings[iRF - THESAURUSACCESSION];
                    ...
                    if (item.PipeType == PipeType.Y1L)
                    {
                        if (item.Hangings.All(x => x.WaterBucket is null))
                        {
                            for (int i = item.Items.Count - THESAURUSACCESSION; i >= QUOTATIONSHAKES; --i)
                            {
                                if (item.Items[i].Exist)
                                {
                                    var hanging = item.Hangings.TryGet(i + THESAURUSACCESSION);
                                    if (hanging != null)
                                    {
                                        hanging.WaterBucket = WaterBucketItem.TryParse(THESAURUSACCLIMATIZE);
                                    }
                                    break;
                                }
                            }
                ...
                foreach (var kv in pipeInfoDict)
                {
                    ...
                    foreach (var hanging in item.Hangings)
                    {
                        ...
                        {
                            item.HasLineAtBuildingFinishedSurfice = THESAURUSABDOMINAL;
                        }
                    }
                }
            }
            {
                ...
                foreach (var kv in pipeInfoDict)
                {
                    ...
                    if (getHasWaterSealingWell(label))
                    {
                        ...
                        item.OutletFloor = ACCLIMATIZATION;
                    }
                    ...
                    {
                        if (testExist(label, ACCLIMATIZATION))
                        {
                            if (getIsSpreading(label, ACCLIMATIZATION))
                            {
            ...
            foreach (var kv in pipeInfoDict)
            {
                var label = kv.Key;
                var item = kv.Value;
                if (item.OutletType == OutletType.散排 && item.OutletFloor == INTERCHANGEABLY)
                {
                    item.HasLineAtBuildingFinishedSurfice = THESAURUSABDOMEN;
                }
                if (item.OutletType == OutletType.散排)
                {
                    foreach (var h in item.Hangings)
                    {
                        ...
                        {
                            h.HasCheckPoint = THESAURUSABDOMEN;
                        }
                    }
                }
            }
            ...
            foreach (var kv in pipeInfoDict)
            {
                var label = kv.Key;
                var item = kv.Value;
                if (item.FloorDrainsCountAt1F == QUOTATIONSHAKES)
                {
                    foreach (var h in item.Hangings)
                    {
                        if (h.Storey == ACCLIMATIZATION)
                        {
                            item.FloorDrainsCountAt1F = h.FloorDrainsCount;
                            h.FloorDrainsCount = QUOTATIONSHAKES;
                        }
                    }
                }
            }
            ...
            var pipeGroupItems5 = new List<RainGroupedPipeItem>();
            ...
            pipeGroupItems5 = pipeGroupItems5.OrderBy(x => x.Labels.First()).ToList();
            ...
            pipeGroupItems.AddRange(pipeGroupItems5);
        ...
        public class Cmd
        {
            ...
            public void Run()
            {
                ...
                {
                    var geo = kv.Value.Buffer(THESAURUSACCLAIM);
                    geo.UserData = kv.Key;
                    return geo;
                }).ToList();
                ...
                {
                    var seg1 = kv.Value.Offset(QUOTATIONSHAKES, THESAURUSACCESSION);
                    var seg2 = kv.Value.Offset(QUOTATIONSHAKES, HEIGHT - THESAURUSACCESSION);
                ...
                for (int j = QUOTATIONSHAKES; j < COUNT; j++)
                {
                    ...
                    var shouldDrawAringSymbol = gpItem.PipeType != PipeType.Y1L && (gpItem.PipeType == PipeType.NL ? (viewModel?.Params?.HasAiringForCondensePipe ?? THESAURUSABDOMINAL) : THESAURUSABDOMINAL);
                    ...
                    void handlePipeLine(ThwPipeLine thwPipeLine, PipeRunLocationInfo[] infos)
                    {
                        ...
                        var hasDrawedAiringSymbol = THESAURUSABDOMEN;
                        ...
                        for (int i = end; i <= start; i++)
                        {
                            ...
                            if (gpItem.OutletType == OutletType.散排)
                            {
                                ...
                                if (gpItem.OutletFloor == storey)
                                {
                                    ...
                                    var seg = new GLineSegment(basePt, basePt.OffsetY(THESAURUSACQUISITIVE));
                                    DrawDimLabel(seg.StartPoint, seg.EndPoint, new Vector2d(THESAURUSABSTRACTED, QUOTATIONSHAKES), THESAURUSALLOWANCE, ELECTROMAGNETIC);
                                    DrawLabel(seg.EndPoint.ToPoint3d(), NEUROTRANSMITTER + storey, storey == INTERCHANGEABLY ? -THESAURUSADDICTION - ThWSDStorey.RF_OFFSET_Y : -THESAURUSADDICTION);
                                    if (!run.HasLongTranslator && !run.HasShortTranslator)
                                    {
                        ...
                        for (int i = start; i >= end; i--)
                        {
                            ...
                            if (storey == null) continue;
                            {
                                ...
                                }
                                {
                                    if (storey == ACCLIMATIZATION)
                                    {
                                        ...
                                        if (gpItem.OutletWrappingPipeRadius != null)
                                        {
                                            ...
                                            DrawStoreyHeightSymbol(p3, THESAURUSABSENCE, gpItem.OutletWrappingPipeRadius);
                                        }
                                        ...
                                        else
                                        {
                                            ...
                                            if (_run != null)
                                            {
                                                if (gpItem == null)
                                                {
                                                    if (_run != null)
                                                    {
                                                        var vecs = new List<Vector2d> { new Vector2d(QUOTATIONSHAKES, -ACCOMMODATIONAL), new Vector2d(-THESAURUSACOLYTE, -THESAURUSACOLYTE), new Vector2d(-THESAURUSACQUAINT, QUOTATIONSHAKES) };
                                                        var p = info.EndPoint;
                                                        var segs = vecs.ToGLineSegments(p);
                                                        drawDomePipes(segs);
                                                        segs = new List<Vector2d> { new Vector2d(QUOTATIONSHAKES, -THESAURUSADDICTION), new Vector2d(-ACRIMONIOUSNESS, QUOTATIONSHAKES) }.ToGLineSegments(segs.Last().EndPoint);
                                                        foreach (var line in DrawLineSegmentsLazy(segs))
                                                        {
                                                            Dr.SetLabelStylesForRainNote(line);
                                                        }
                                                        {
                                                            var t = DrawTextLazy(THESAURUSACQUAINTANCE, POLYOXYMETHYLENE, segs.Last().EndPoint.OffsetXY(INCOMPREHENSIBLE, INCOMPREHENSIBLE));
                                                            Dr.SetLabelStylesForRainNote(t);
                                                        }
                                                    }
                                ...
                                }
                                {
                                    ...
                                    var fixW2 = gpItem.Hangings.All(x => (x?.FloorDrainWrappingPipesCount ?? QUOTATIONSHAKES) == QUOTATIONSHAKES) ? ACCOMMODATIONAL : QUOTATIONSHAKES;
                                    var fixW = UNCHRONOLOGICAL - fixW2;
                                    ...
                                    if (hanging.FloorDrainsCount > QUOTATIONSHAKES)
                                    {
                                        ...
                                        if (hanging.FloorDrainsCount == THESAURUSACCESSION)
                                        {
                                            if (!(storey == ACCLIMATIZATION && (gpItem.HasSingleFloorDrainDrainageForWaterWell || gpItem.HasSingleFloorDrainDrainageForRainPort)))
                                            {
                                                ...
                                                if (gpItem.Items.TryGet(i - THESAURUSACCESSION).HasLong)
                                                {
                                                    if (runs[i - THESAURUSACCESSION].IsLongTranslatorToLeftOrRight)
                                                    {
                                                        var p = vecs.GetLastPoint(bsPt.OffsetY(-THESAURUSACQUIRE - THESAURUSACQUIREMENT) + v);
                                                        var _vecs = new List<Vector2d> { new Vector2d(-THESAURUSACOLYTE, QUOTATIONSHAKES), new Vector2d(-THESAURUSACCOUNTABLE, -THESAURUSACCRUE), new Vector2d(QUOTATIONSHAKES, -UNSUBSTANTIALITY), new Vector2d(THESAURUSACCOUNTABLE, -THESAURUSACCRUE) };
                                                        var segs = _vecs.ToGLineSegments(p);
                                                        _DrawFloorDrain(p, THESAURUSABDOMINAL);
                                                        drawDomePipes(segs);
                                                        tryDrawWrappingPipe(p.OffsetX(ACQUISITIVENESS));
                                                        var __vecs = new List<Vector2d> { new Vector2d(-ADRENOCORTICOTROPHIC, QUOTATIONSHAKES), new Vector2d(QUOTATIONSHAKES, -THESAURUSANIMAL), new Vector2d(QUOTATIONSHAKES, -THESAURUSACCURATE) };
                                                        ...
                                                        DrawDimLabel(seg.StartPoint, seg.EndPoint, new Vector2d(-THESAURUSABSTRACTED, QUOTATIONSHAKES), ANDROMONOECIOUS, ELECTROMAGNETIC);
                                                        ok = THESAURUSABDOMINAL;
                                                    }
                                                }
                                                if (!ok)
                                                {
                                                    var segs = vecs.ToGLineSegments(bsPt.OffsetY(-THESAURUSACQUIRE - THESAURUSACQUIREMENT) + v);
                                                    drawDomePipes(segs);
                                                    var p = segs.Last().EndPoint;
                                                    _DrawFloorDrain(p, THESAURUSABDOMINAL);
                                                    tryDrawWrappingPipe(p.OffsetX(ACQUISITIVENESS));
                                    ...
                                    if (hanging.HasCondensePipe)
                                    {
                                        ...
                                        {
                                            var v = new Vector2d(QUOTATIONSHAKES, -INCOMPREHENSIBLE);
                                            void f(double offsetY)
                                            {
                                                ...
                                                _DrawCondensePipe(p3.OffsetXY(-INATTENTIVENESS, INATTENTIVENESS));
                                                drawDomePipes(segs);
                                        ...
                                        {
                                            var v = new Vector2d(QUOTATIONSHAKES, -THESAURUSACCURATE);
                                            var segs = vecs.ToGLineSegments((info.StartPoint + v).OffsetY(-THESAURUSACQUIT));
                                            var p1 = segs.Last().EndPoint;
                                            var p2 = p1.OffsetX(w);
                                            var p3 = p1.OffsetY(h);
                                            var p4 = p2.OffsetY(h);
                                ...
                                {
                                    var h = Math.Round(HEIGHT / THESAURUSANIMATE * THESAURUSACCUSE);
                        ...
                        if (list.Count > QUOTATIONSHAKES)
                        {
                            ...
                            var h = THESAURUSABSTRACTED - THESAURUSADMINISTER;
                            foreach (var pt in list)
                            {
                                if (pt.Y != my) continue;
                                var ok = THESAURUSABDOMEN;
                                {
                                    ...
                                    if (storey != null)
                                    {
                                        ...
                                        {
                                            linesKillers.Add(new GLineSegment(pt.OffsetY(-THESAURUSABANDON), pt.OffsetY(-THESAURUSABANDON).OffsetX(THESAURUSACERBIC)).ToLineString());
                                        }
                                        ok = THESAURUSABDOMINAL;
                                    }
                                }
                                if (!ok)
                                {
                                    ...
                                    if (storey != null)
                                    {
                                        ...
                                        if (shouldDrawAringSymbol)
                                        {
                                            ...
                                            _DrawAiringSymbol(p.OffsetY(h));
                                            linesKillers.Add(new GLineSegment(pt, pt.OffsetX(THESAURUSACERBIC)).ToLineString());
                                        }
                                        else
                                        {
                                            linesKillers.Add(new GLineSegment(pt, pt.OffsetX(THESAURUSACERBIC)).ToLineString());
                    ...
                    }
                    {
                        ...
                        }
                        {
                            ...
                            var _storeys = new string[] { _allSmoothStoreys.GetAt(QUOTATIONSHAKES), _allSmoothStoreys.GetLastOrDefault(THESAURUSACCIDENT) }.SelectNotNull().Distinct().ToList();
                            if (_storeys.Count == QUOTATIONSHAKES)
                            {
                    ...
                    if (linesKillers.Count > QUOTATIONSHAKES)
                    {
        ...
        {
            DrawBlockReference(blkName: THESAURUSANAESTHETIC, basePt: basePt.ToPoint3d(),
         cb: br =>
         {
...
#pragma warning disable
        ...
        public static void CreateDrawingDatas(RainGeoData geoData, RainCadData cadDataMain, List<RainCadData> cadDatas, out string logString, out List<RainDrawingData> drDatas, List<KeyValuePair<string, Geometry>> roomData)
        {
            ...
            for (int storeyI = QUOTATIONSHAKES; storeyI < cadDatas.Count; storeyI++)
            {
                ...
                var item = cadDatas[storeyI];
                {
                    ...
                    var lines = GeoFac.AutoConn(item.WLines.Where(x => x.Length > QUOTATIONSHAKES).Distinct().ToList().Select(cadDataMain.WLines).ToList(geoData.WLines).ToList(),
                    ...
                    {
                        {
                            ...
                            foreach (var label in item.Labels)
                            {
                                ...
                                if (IsGravityWaterBucketLabel(text))
                                {
                                    ...
                                    {
                                        var labelline = lst[QUOTATIONSHAKES];
                                        if (f(GeoFac.CreateGeometry(label, labelline)).Count == QUOTATIONSHAKES)
                                        {
                                            var lines = ExplodeGLineSegments(labelline);
                                            var points = GeoFac.GetLabelLineEndPoints(lines.Distinct(new GLineSegment.EqualityComparer(THESAURUSACCUSE)).ToList(), label, radius: THESAURUSACCUSE);
                                            foreach (var pt in points)
                                            {
                                                ...
                                                var r = GRect.Create(pt, INATTENTIVENESS);
                ...
                }
                {
                    ...
                    foreach (var kv in lbDict)
                    {
                        ...
                        if (kv.Value.Contains(ALSOAMBIVALENCY))
                        {
                            var floor = THESAURUSAMBITIOUS;
                            var pipe = kv.Key;
                            var pipes = getOkPipes().ToList();
                            var wlines = wlinesf(pipe);
                            if (wlines.Count == THESAURUSACCESSION)
        ...
        private static void DrawGeoData(RainGeoData geoData, RainCadData cadDataMain, int storeyI, RainCadData item)
        {
            ...
            {
                DrawRectLazy(geoData.WaterSealingWells[cadDataMain.WaterSealingWells.IndexOf(o)]).ColorIndex = THESAURUSADAPTATION;
            }
    ...
    public class ThRainService
    {
        ...
        public static void PreFixGeoData(RainGeoData geoData)
        {
            ...
            {
                var v = INCOMPREHENSIBLE;
                for (int i = QUOTATIONSHAKES; i < geoData.WrappingPipes.Count; i++)
                {
                    var wp = geoData.WrappingPipes[i];
                    if (wp.Width > v * THESAURUSACCIDENT)
                    {
    ...
    public static class THRainService
    {
        ...
        public const int THESAURUSACERBIC = 3000;
        ...
        public const string THESAURUSAMIABLE = "$W-DRAIN-2";
        ...
        public const string ANAKHRONIZESTHAI = "$W-DRAIN-5";
        public const string THESAURUSANAEMIC = "-1F";
        public const string ANAESTHESIOLOGY = "W-DRAIN-1";
        public const string QUOTATIONGENERAL = "W-DRAIN-2";
        public const string ANAESTHETICALLY = "W-DRAIN-5";
        public const string THESAURUSANAESTHETIC = "水封井";
        public const string ANDROMONOECIOUS = "≥600";
        public const int UNSUBSTANTIALITY = 331;
        public const int THESAURUSANIMAL = 821;
        ...
        {
            if (label == null) return THESAURUSABDOMEN;
            return IsRainLabel(label) || label.Contains(THESAURUSADMINISTRATIVE);
        }
        ...
        {
            if (string.IsNullOrEmpty(label)) return THESAURUSABDOMEN;
