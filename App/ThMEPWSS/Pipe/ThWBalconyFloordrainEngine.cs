...
namespace ThMEPWSS.Pipe
{
    public class ThWBalconyFloordrainEngine : IDisposable
    {
        ...
        public void Run(List<BlockReference> bfloordrain, Polyline bboundary, Polyline rainpipe, Polyline downspout, BlockReference washingmachine, Polyline device, Polyline device_other, Polyline condensepipe, BlockReference bbasinline)
        {
            ...
            {   if(device_other==null)
                ...
                if (GeomUtils.PtInLoop(bboundary, center))
                {
                    ...
                    {
                        if (Floordrain_washing[0].Position.DistanceTo(b_floordrain.Position) < 700)
                        {
                    ...
                    if (bbasinline != null && bbasinline.Position.DistanceTo(washingmachine.Position) < 900)
                    {
                ...
                else
                {
                    foreach (var b_floordrain in Floordrain)
                    {
                        if (Floordrain_washing[0].Position.DistanceTo(b_floordrain.Position) < 700)
                        {
                            Downspout_to_Floordrain = Line_Addvertices(bboundary, center, Floordrain_washing, washingmachine, device_other, b_floordrain.Position);
                            break;
                        }
                    }
                    ...
                    if (bbasinline != null && bbasinline.Position.DistanceTo(washingmachine.Position) < 900)
                    {
                        Bbasinline_to_Floordrain= Line_Addvertices1(bboundary, center, Floordrain_washing, washingmachine, device_other, Bbasinline_Center[0]);                     
                    }
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        Downspout_to_Floordrain = Line_vertices(bboundary, downspout.GetCenter(), Floordrain_washing, washingmachine, device_other);
            ...
            if (rainpipe != null)
            {
                ...
                else
                {
                    ...
                    {
                        if (rainpipe.GetCenter().DistanceTo((criticalfloordrain.Position)) <= 2000/*800*/)
                        {
                            Rainpipe_to_Floordrain = Line_vertices_rainpipe(bboundary, rainpipe.GetCenter(), criticalfloordrain, device);
                            break;
        ...
        private static Point3dCollection Line_vertices(Polyline bboundary, Point3d center_spout, List<BlockReference> Floordrain_washing, BlockReference washingmachine, Polyline device)
        {
            ...
            if (perpendicular_point2.DistanceTo(pts[0]) < dmin)
            {
...

                {
                    ...
                    {
                        vertices.Add(Floordrain_washing[0].Position + 50 * Floordrain_washing[0].Position.GetVectorTo(perpendicular_point3).GetNormal());
                        ...
                        vertices.Add(center_spout + 50 * center_spout.GetVectorTo(perpendicular_point1).GetNormal());
                    }
                    else
                    {
                        vertices.Add(Floordrain_washing[0].Position + 50 * Floordrain_washing[0].Position.GetVectorTo(perpendicular_point3).GetNormal());
            ...
            else
            {
                ...
                if (perpendicular_point4.DistanceTo(pts1[0]) >= 200)
                {
                    if (pts1[0].DistanceTo(center_spout) >= 50)
                    {
                ...
                {
                    if (perpendicular_point4.DistanceTo(pts1[0]) <= 150)
                    {
                        vertices.Add(center_spout + 50 * (perpendicular_point.GetVectorTo(perpendicular_point1).GetNormal()));
                        vertices.Add(center_spout + (200 - perpendicular_point4.DistanceTo(pts1[0])) * (perpendicular_point.GetVectorTo(perpendicular_point1).GetNormal()));
                        vertices.Add(pts1[0] + (200 - perpendicular_point4.DistanceTo(pts1[0])) * perpendicular_point4.GetVectorTo(pts1[0]).GetNormal());
                        vertices.Add(Floordrain_washing[0].Position + 50 * perpendicular_point4.GetVectorTo(pts1[0]).GetNormal());
                    }
                    else
                    {
                        var temp = center_spout + (200 - perpendicular_point4.DistanceTo(pts1[0])) * (perpendicular_point.GetVectorTo(perpendicular_point1).GetNormal());
                        var temp1 = pts1[0] + (200 - perpendicular_point4.DistanceTo(pts1[0])) * perpendicular_point4.GetVectorTo(pts1[0]).GetNormal();
                        var tts = new Point3dCollection();
                        Line templine = new Line(temp, temp1);
                        Circle circletemp = new Circle() { Center = center_spout, Radius = 50 };
                        circletemp.IntersectWith(templine, Intersect.ExtendArgument, tts, (IntPtr)0, (IntPtr)0);
        ...
        private static Point3d new_downspout(Polyline bboundary, Polyline rainpipe, List<BlockReference> Floordrain_washing, Polyline device)//新的排水管井
        {
            ...
            double dmax1 = double.MaxValue;
            double dmax2 = double.MaxValue;
            int a = 0;
            //device距离最近点
            ...
            {
                if (dmax1 > (Floordrain_washing[0].Position.DistanceTo(vertices_boundary[i])))
                {
                    dmax1 = (Floordrain_washing[0].Position.DistanceTo(vertices_boundary[i]));
                    b = i;
                }
            }
            int c = 0;
            ...
            for (int i = 0; i < vertices_boundary.Count; i++)
            {
                if (dmax2 > (rainpipe.GetCenter().DistanceTo(vertices_boundary[i])))
                {
                    dmax2 = (rainpipe.GetCenter().DistanceTo(vertices_boundary[i]));
                    c = i;
                }
            }
            //判断关键边
        ...
        private static Point3dCollection Line_vertices_rainpipe(Polyline bboundary, Point3d center_rainpipe, BlockReference Floordrain, Polyline device)
        {
            ...
            if (perpendicular_point2.DistanceTo(pts[0]) < dmin)
            {
                if (perpendicular_point2.DistanceTo(pts[0]) >= 200)
                {
                    if (pts[0].DistanceTo(center_rainpipe) >= 50)
                    {

                        vertices.Add(center_rainpipe + 50 * (center_rainpipe.GetVectorTo(perpendicular_point).GetNormal()));
                        ...
                        vertices.Add(Floordrain.Position + 50 * perpendicular_point1.GetVectorTo(perpendicular_point).GetNormal());
                    }
                    else
                    {
                ...
                else
                {
                    if (perpendicular_point2.DistanceTo(pts[0]) <= 150)
                    {
                        ...
                        vertices.Add(pts[0] + (200 - perpendicular_point2.DistanceTo(pts[0])) * perpendicular_point2.GetVectorTo(pts[0]).GetNormal());
                        vertices.Add(Floordrain.Position + 50 * perpendicular_point2.GetVectorTo(pts[0]).GetNormal());
                    }
                    else
                    {
            ...
            else
            {
                ...
                {
                    if (pts1[0].DistanceTo(center_rainpipe) >= 50)
                    {
                        vertices.Add(center_rainpipe + 50 * (center_rainpipe.GetVectorTo(perpendicular_point).GetNormal()));
                        vertices.Add(perpendicular_point);
                        vertices.Add(Floordrain.Position + (perpendicular_point1.GetVectorTo(perpendicular_point).GetNormal()) * (perpendicular_point.DistanceTo(perpendicular_point1) - perpendicular_point1.DistanceTo(Floordrain.Position)));
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        vertices.Add(temp1);
                        vertices.Add(Floordrain.Position + 50 * perpendicular_point3.GetVectorTo(pts1[0]).GetNormal());
                    }
                }
            }
            return vertices;
        ...
        private static Point3dCollection Getvertices(Polyline bboundary, BlockReference washingmachine, Point3d bbasinline, List<BlockReference> Floordrain_washing)
        {
            ...
            vertices.Add(bbasinline + (bbasinline).GetVectorTo(vertices[0]).GetNormal() * 50);
            return vertices;
        }    
        ...
        private static Point3dCollection Line_Addvertices1(Polyline bboundary, Point3d center_spout, List<BlockReference> Floordrain_washing, BlockReference washingmachine, Polyline device, Point3d b_floordrain)
        {
            ...
            var perpendicular_point1 = linespecific.ToCurve3d().GetClosestPointTo(b_floordrain).Point;//洗衣机所在边洗衣机地漏垂点            
            var perpendicular_point3 = device.ToCurve3d().GetClosestPointTo(b_floordrain).Point;//排水井在洗衣机所在边垂点                                          
            vertices.Add(b_floordrain+50*b_floordrain.GetVectorTo(perpendicular_point3).GetNormal());
            vertices.Add(perpendicular_point3);
            vertices.Add(perpendicular_point3 + (center_spout.DistanceTo(perpendicular_point) - perpendicular_point1.DistanceTo(perpendicular_point3)) * (perpendicular_point.GetVectorTo(center_spout).GetNormal() + (perpendicular_point1.GetVectorTo(perpendicular_point).GetNormal())));
            vertices.Add(center_spout + 50 * center_spout.GetVectorTo(vertices[2]).GetNormal());
            return vertices;
        }
        ...
        private static Point3dCollection GetBbasinline_Center(Polyline bboundary,  List<BlockReference> Floordrain_washing, BlockReference washingmachine,Point3d b_floordrain)
        {
            ...
            {
                if (b_floordrain.DistanceTo(bboundary.Vertices()[num+1]) > 200)
                {
                    var center = b_floordrain + dis * perpendicular_point2.GetVectorTo(Floordrain_washing[0].Position).GetNormal() + 200 * perpendicular_point2.GetVectorTo(perpendicular_point1).GetNormal();
                    if (GeomUtils.PtInLoop(bboundary, center))
                    { vertices.Add(center); }
