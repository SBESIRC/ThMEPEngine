...
pt,
    ...
    public static class GeometryFac
    {
        ...
        public static Polygon CreateCirclePolygon(Point2d center, double radius, int numPoints = 6)
        {
            var shapeFactory = new NetTopologySuite.Utilities.GeometricShapeFactory(ThCADCoreNTSService.Instance.GeometryFactory)
            {
                NumPoints = numPoints,
    ...
    public class DrainageCadData
    {
        ...
        public static DrainageCadData Create(DrainageSystemGeoData data)
        {
            var bfSize = 10;
            ...
            o.DLines.AddRange(data.DLines.Select(x => x.Buffer(bfSize)));
            ...
            o.WaterPorts.AddRange(data.WaterPorts.Select(x => x.ToPolygon()));
            o.WashingMachines.AddRange(data.WashingMachines.Select(x => x.ToPolygon()));
            return o;
        }
    ...
    public class DrainageSystemGeoData
    {
        ...
        public DrainageSystemGeoData Clone()
        {
            return (DrainageSystemGeoData)MemberwiseClone();
        }
        public DrainageSystemGeoData DeepClone()
        {
            return this.ToCadJson().FromCadJson<DrainageSystemGeoData>();
        }
    }
    ...
    public class ThDrainageSystemServiceGeoCollector
    {
        ...
        public DrainageSystemGeoData geoData;
    ...
    public class DrainageService
    {
        ...
        public static List<Geometry> GetKitchenOnlyFLs(List<Geometry> FLs, List<Geometry> kitchens, List<Geometry> nonames, List<Geometry> balconies)
        {
            ...
            foreach (var fl in FLs)
            {
                ...
                {
                    return GetEndPoints(fl.EnvelopeInternal.ToGRect().Center.ToNTSPoint());
                }
                bool test1()
                {
                    return kitchensGeo.Intersects(GeometryFac.CreateCirclePolygon(fl.EnvelopeInternal.ToGRect().Center.ToPoint3d(), 500, 36));
                }
                bool test2()
                {
        ...
        public static List<Geometry> GetBalconyOnlyFLs(List<Geometry> FLs, List<Geometry> kitchens, List<Geometry> nonames, List<Geometry> balconies)
        {
            ...
            foreach (var fl in FLs)
            {
                ...
                bool test2()
                {
                    return !GeometryFac.CreateCirclePolygon(fl.EnvelopeInternal.ToGRect().Center.ToPoint3d(), 500, 36).Intersects(kitchensGeo);
                }
                bool test3()
                {
                    endpoints ??= _GetEndPoints();
    ...
    public class ThDrainageService
    {
        public static void DrawDrainageSystemDiagram2()
        {
            ...
            using (var tr = new DrawingTransaction(adb))
            {
                try
                {
                    ...
                    var geoData = new DrainageSystemGeoData();
                    geoData.Init();
        ...
        public static void ConnectLabelToLabelLine(DrainageSystemGeoData geoData)
        {
        ...
        public static void PreFixGeoData(DrainageSystemGeoData geoData)
        {
            ...
            public class ToiletGrouper
            {
                ...
                public static List<ToiletGrouper> CollectFLTLs(List<ToiletGrouper> group, List<Geometry> FLs, List<Geometry> TLs)
                {
                    ...
                    foreach (var fl in FLs)
                    {
                        ...
                        var range = GeometryFac.CreateCirclePolygon(fl.EnvelopeInternal.ToGRect().Center.ToPoint3d(), 300, 36);
                        var lst = f(range);
                        if (lst.Count > 0)
                        {
                            tls.AddRange(lst);
                ...
                public static List<ToiletGrouper> DoGroup(List<Geometry> PLs, List<Geometry> TLs, List<Geometry> DLs)
                {
                    ...
                    {
                        hs.Add(pl);
                        var range = GeometryFac.CreateCirclePolygon(pl.EnvelopeInternal.ToGRect().Center.ToPoint3d(), 300, 12);
                        //在每一根PL的每一层300范围内找TL
