...
namespace ThMEPWSS.UndergroundFireHydrantSystem.Service
{
    ...
    {
        public static void GetMainLoop(ref FireHydrantSystemOut fireHydrantSysOut, List<List<Point3dEx>> mainPathList, FireHydrantSystemIn fireHydrantSysIn)
        {
            using ( AcadDatabase acadDatabase = AcadDatabase.Active())
            {
                ...
                fireHydrantSysOut.InsertPoint = new Point3d(stPt1.X, stPt1.Y + 20000 * index, 0); ;
        ...
        public static void GetBranch(ref FireHydrantSystemOut fireHydrantSysOut, Dictionary<Point3dEx, List<List<Point3dEx>>> branchDic, FireHydrantSystemIn fireHydrantSysIn)
        {
            ...
            foreach (var pt in branchDic.Keys)//对于支路的每一个起始点
            {
                ...
                if (branchDic[pt].Count == 1)//单支路
                {
                    ...
                    if(fireHydrantSysIn.termPointDic.ContainsKey(branchDic[pt].First().Last()))
                    {
        ...
        private static void GetBranchType3(ref FireHydrantSystemOut fireHydrantSysOut, Point3dEx stpt, List<List<Point3dEx>> ptList, FireHydrantSystemIn fireHydrantSysIn)
        {
            ...
            string pipeNumber1 = "";
            string pipeNumber2 = "";
            if(ptList.First().Last().Equals(new Point3dEx(1017702.57435943, 1103873.35390667, 0)))
            {
                ;
            }
            if(ptList.First().Last().Equals(new Point3dEx(1016656.81693602, 1104239.71135726, 0)))
            {
                ;
            }
            if (fireHydrantSysIn.termPointDic.ContainsKey(ptList.First().Last()))
            {
                pipeNumber1 = fireHydrantSysIn.termPointDic[ptList.First().Last()].PipeNumber;//立管标号 1
            }
            if (fireHydrantSysIn.termPointDic.ContainsKey(ptList.Last().Last()))
            {
                pipeNumber2 = fireHydrantSysIn.termPointDic[ptList.Last().Last()].PipeNumber;//立管标号 2
