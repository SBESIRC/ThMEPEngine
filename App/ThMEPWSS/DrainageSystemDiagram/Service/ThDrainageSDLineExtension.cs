...
{
    /// <summary>
    /// 和 ThMEPEngineCore.Algorithm ：ThMEPLineExtension 基本一样
    /// 除了LineMerge方法 条件多了
    /// tuple.Item1 != tuple.Item2
    /// 没经过测试怕影响别的代码故不直接改原代码
    /// 删除了本project没用到的方法 IsOverlapByTol 
    /// </summary>
    public static class ThDrainageSDLineExtension
    {
        ...
        //tuple.Item1 != tuple.Item2 和原方法ThMEPEngineCore.Algorithm ：ThMEPLineExtension 不一样
        private static List<Line> LineMerge(List<Line> lines, double DistGap2Extend, double DistGap2Merge, double AngleTolerance)
        {
            ...
            {
                var tuple = lists[i];
                if ((lists.Find(o => lists.IndexOf(o) < i && o == tuple) == null) && tuple.Item1 != tuple.Item2) // 判断是否有重复操作的直线对
                {
