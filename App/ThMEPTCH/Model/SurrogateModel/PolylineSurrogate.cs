...
namespace ThMEPTCH.Model.SurrogateModel
{
    ...
    {
        public PolylineSurrogate(List<PointCollection> pts, bool closed) : this()
        {
            this.Points = pts;
        ...
        [ProtoMember(1)]
        public List<PointCollection> Points { get; set; }
        ...
        {
            if (polyline.IsNull())
                return new PolylineSurrogate(new List<PointCollection>(), false);
            ...
            for (int k = 0; k < segments.Count(); k++)
            {
                ...
                if (segment.IsLinear)
                {
                    ...
                    pts.Add(new PointCollection(new List<Point3DSurrogate>() { new Point3DSurrogate(line.StartPoint.X, line.StartPoint.Y, 0) }));

                    if (!polyline.Closed && k == segments.Count() - 1)
                    {
                        pts.Add(new PointCollection(new List<Point3DSurrogate> { new Point3DSurrogate(line.EndPoint.X, line.EndPoint.Y, 0) }));
                    }
                }
                else
                {
                    ...
                    var points = new PointCollection(new List<Point3DSurrogate>() { new Point3DSurrogate(arc.StartPoint.X, arc.StartPoint.Y, 0) });
                    ...
                    if (!polyline.Closed && k == segments.Count() - 1)
                    {
                        pts.Add(new PointCollection(new List<Point3DSurrogate> { new Point3DSurrogate(arc.EndPoint.X, arc.EndPoint.Y, 0) }));
                    }
                }
            }
    ...
    {
        public PointCollection(List<Point3DSurrogate> pts) : this()
        {
            this.Points = pts;
        }
